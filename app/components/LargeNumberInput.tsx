...
export const LargeNumberInput = ({ amount, orderType, setAmount, side, symbol }) => {
  const inputTextSize = String(amount).length < 4
    ? "text-8xl"
  ...
  const stockLabelSize = String(amount).length < 3
    ? "text-5xl"
    : String(amount).length < 5
      ? "text-4xl"
      : String(amount).length < 6
    ...
    <div>
      <div className="relative flex items-center justify-center mt-1 rounded-md">
        ...
        <input
          ...
          id="price"
          name="price"
          className={`w-full pointer-cursor text-center border-none h-72 ${inputTextSize} placeholder-brand text-brand font-primary leading-6 ${!amount ? "focus:animate-pulse" : ""} focus:appearance-none focus:border-none focus:ring-0`}
          placeholder="0"
