...
const incrementInvestors = async (change, context) => {
    ...
    if (!change.before.exists) {
        ...
        index_js_1.firestore.doc(`groups/${groupName}`).update({ investorCount: index_js_2.increment(1) });
    }
...
const subscribeToGroupTopic = async (change, context) => {
    ...
    const fcmTokens = await getInvestorFcmTokens_js_1.getInvestorFcmTokens(groupName);
...
const initialDeposit = async (snapshot, context) => {
    const { groupName } = context.params;
    const fundClient = new index_js_3.FundingApi(index_js_3.config(process.env.ALPACA_KEY, process.env.ALPACA_SECRET));
    ...
    if (totalAmount > 0 && alpacaACH && alpacaAccountId) {
        try {
            ...
            fundingRef.set({
                subscriptionAmount,
                initialDeposit,
                startDate: index_js_2.serverTimestamp(),
                deposits: [
                    {
                        amount: totalAmount,
