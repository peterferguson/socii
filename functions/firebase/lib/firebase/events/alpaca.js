...
async function handleEvents(req, res) {
    ...
    if (!(since || until || since_id || until_id)) {
        since_id = await firestore_1.getLatestEventId(type);
    }
    ...
    const baseServer = {
        ...
        ...index_js_1.config(functions.config().alpaca.key, functions.config().alpaca.secret)
            .baseServer,
    };
    if (type in alpacaEventEndpoints) {
        try {
            console.log(`${baseServer.url}/events/${alpacaEventEndpoints[type]}?${query}`);
            await sseGetRequest_1.sseGetRequest(`${baseServer.url}/events/${alpacaEventEndpoints[type]}?${query}`, exports.responseCallback(res, type, since_id));
        }
        catch (e) {
            console.log(e);
        }
...
const responseCallback = (res, type, since_id) => (response) => {
    ...
    response.on("end", async () => {
        response.destroy();
        await firestore_1.storeEvents(type, Buffer.concat(data).toString(), since_id);
        res
            .status(200)
