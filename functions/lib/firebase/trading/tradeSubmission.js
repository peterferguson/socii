...
const tradeSubmission = async (data, context) => {
    (0, verifyUser_1.verifyUser)(context);
    const verifiedData = await (0, verifyTradeSubmissionContent_1.verifyTradeSubmissionContent)(data, context);
    ...
    const canAffordTrade = await Promise.all(investors.map(async (investor) => {
        ...
        const { cash } = await (0, getAlpacaBuyPower_js_1.getAlpacaBuyPower)(data.alpacaAccountId);
        return {
    ...
    if (!canAffordTrade.every(({ isAffordable }) => isAffordable))
        return submittedFromCallable
            ...
            : await streamClient_js_1.streamClient
                ...
                .sendMessage((0, notEnoughBuyingPowerMessage_js_1.notEnoughBuyingPowerMessage)(verifiedData.username));
    // - Create trade document
    ...
    tradeRef.set({
        ...
        timestamp: (0, index_js_1.serverTimestamp)(),
    });
    firebase_functions_1.logger.log(verifiedData);
    if (investorCount > 1) {
        // - Send confirmation message into chat
        const message = (0, confirmInvestmentMML_1.confirmInvestmentMML)({
