...
const alphaVantageQuery = async (data, context) => {
    ...
    if (dataSnapshot.exists) {
        return (0, filterKeys_1.filterKeys)(dataSnapshot.data(), data.queryFields);
    }
    else {
        const response = await alphaVantageSummary(data.tickerSymbol);
        const exchange = response.Exchange || "";
        const cleanResponse = (0, cleanJsonResponse_1.cleanJsonResponse)(response);
        // * Store result in firestore
        const batch = index_js_1.firestore.batch();
        batch.update(tickerRef, { exchange: exchange });
        batch.set(dataRef, {
            ...cleanResponse,
            lastUpdate: (0, index_js_2.serverTimestamp)(),
        });
        batch.commit();
        return (0, filterKeys_1.filterKeys)(cleanResponse, data.queryFields);
    }
};
