...
export const useWatchedChannels = () => {
  ...
  const [directMessagingConversations, setDirectMessagingConversations] = useState([])
  ...
  useEffect(() => {
    function handleEvents(e) {
      if (e.type === "message.new") {
        ...
        const channelReadIndex = readChannels.findIndex(channel => channel.cid === cid)
        ...
        if (directMessagingConversationIndex >= 0) {
          ...
          const channel = directMessagingConversations[directMessagingConversationIndex]
          directMessagingConversations.splice(directMessagingConversationIndex, 1)
      ...
      if (e.type === "message.read") {
        ...
        const cid = e.cid
        // get channel index
        const channelIndex = unreadChannels.findIndex(channel => channel.cid === cid)
        if (channelIndex < 0) {
          return
        }
        ...
        if (!channel.data.name) {
          setDirectMessagingConversations([channel, ...directMessagingConversations])
        } else {
