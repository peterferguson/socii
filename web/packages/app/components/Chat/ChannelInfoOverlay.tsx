...
import { Avatar, CircleClose, Delete, User, UserMinus, vh, vw } from "stream-chat-expo"
...
export const ChannelInfoOverlay = (props: ChannelInfoOverlayProps) => {
  ...
  const { overlay, setOverlay } = useChatOverlayContext()
  ...
  const maxWidth = channel
    ? Math.floor(
        width / 8 - Object.keys(channel.state.members || {}).length.toString().length
      )
    : 0
