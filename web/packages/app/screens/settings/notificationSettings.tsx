...
import { getUserNotificationSettings } from "app/lib/firebase/db"
...
const NotificationSettingsScreen = () => {
  ...
  useEffect(() => {
    if (user?.uid)
      getUserNotificationSettings(user.uid).then(r => setInitialSettings(r))
  }, [user.uid])
  ...
  const notificationOptions = [
    {
      title: "Email",
      subTitle:
        "Recieve our newsletter and interesting updates from the investing world.",
      onPress: () => setEmail(!email),
      itemState: email,
    },
    {
      title: "Marketing",
      subTitle: "Hear about our upcoming events, competitions and more.",
      onPress: () => setMarketing(!marketing),
      itemState: marketing,
    },
    {
      title: "Messages",
      subTitle: "Get notifications for chat messages",
      onPress: () => setMessages(!messages),
      itemState: messages,
    },
    {
      title: "Updates",
      subTitle: "Be the first to know about new features and releases",
    ...
    <View style={tw`flex flex-row justify-center`}>
      ...
      >
        ...
        <Text style={tw`text-center text-pink-400 pb-3`}>
          These will be enabled soon - keep an eye on our social media for future
          releases!
        </Text>
              ...
              <View style={tw`flex flex-row justify-left pl-2 pr-2`}>
                <View style={tw`flex justify-center`}>
                  <Pressable
                    style={[
                      {
                        ...tw`w-7 h-7 rounded-md border-2 bg-transparent align-items-center`,
                      },
                      itemState && {
