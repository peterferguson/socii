...
const tradeConfirmation = async (change, context) => {
    ...
    await (0, warnPriceVariationOnMarketClose_1.warnPriceVariationOnMarketClose)(isUSMarketOpen, primaryExchange, latestPrice, symbol, groupName, latestAgreesId);
    ...
    if (tradeData.agreesToTrade.length === investorCount) {
        ...
        switch ((0, determineTradeStatus_1.determineTradeStatus)(postOrder.status) ?? "failed") {
            ...
            case "pending":
                ...
                await channel.sendMessage((0, investmentPendingMML_1.investmentPendingMML)({
                    ...tradeData,
                    tradeId,
...
const getIexPrice = async (symbol) => {
    const { latestPrice, isUSMarketOpen, primaryExchange } = await index_js_2.iexClient.quote(symbol, { filter: "latestPrice,isUSMarketOpen,primaryExchange" });
    return { latestPrice, isUSMarketOpen, primaryExchange };
};
const getAlpacaPrice = async (symbol, tradeType) => {
    const { meta, quotes: { [symbol]: { quote }, }, } = await (0, getRealtimeQuotes_1.getRealtimeQuotes)([symbol]);
    const latestPrice = (0, isSell_1.isSell)(tradeType) ? quote.bp : quote.ap;
    const primaryExchange = (0, isSell_1.isSell)(tradeType) ? quote.bx : quote.ax;
