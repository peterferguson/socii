...
const upsertHolding = async ({ holdingDocRef, tradeData, messageId, qty, privacyOption, }) => {
    ...
    if (holding.exists) {
        ...
        outputData.holdingData = {
            ...
            qty: (0, index_js_1.increment)(sharesIncrement),
            lastUpdated: (0, index_js_1.serverTimestamp)(),
            trades: (0, index_js_1.arrayUnion)(messageId),
        };
    }
    else {
        outputData.type = "set";
...
const getLogoColor = async (assetRef) => {
    const assetData = await index_js_2.firestore.doc(assetRef).get();
